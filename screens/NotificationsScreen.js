...
const NotificationItem = ({ item, index, onAction }) => {
  ...
  return (
    <View style={{ position: 'relative', minHeight: 0 }}>
      ...
      >
        <Feather name="check" size={20} color="#ffffff" style={{ opacity: 0.7 }} />
        <Text style={[styles.swipeText, { opacity: 0.7 }]}>{item.read ? 'Mark Unread' : 'Mark Read'}</Text>
      </Animated.View>
      <Animated.View
        pointerEvents="none"
        style={[
      ...
      >
        <Feather name="trash-2" size={20} color="#ffffff" style={{ opacity: 0.7 }} />
          ...
          {isExpanded && (
            <View style={styles.actionButtons}>
              ...
              >
                ...
                <Text style={styles.actionButtonText}>{item.read ? 'Mark Unread' : 'Mark Read'}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  styles.actionButton,
...
export default function NotificationsScreen() {
  ...
  const fetchNotifications = async () => {
    ...
    const mapped = scheduled.map((n, idx) => ({
